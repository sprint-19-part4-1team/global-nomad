 ...
 *
 * @property {number} activityId - 조회할 체험의 ID. 체험 ID가 없으면 `null`일 수 있습니다.
 ...
 * @param params.activityId - 체험 ID (null이면 API 호출 안함)
 * @param params.year - 조회할 년도 (예: 2026)
 * @param params.month - 조회할 월 1-12 (예: 1, 2, ..., 12)
 ...
 * });
 *
 * // activityId가 null이면 API 호출 안함
 * const { reservations, isLoading } = useMonthlyReservations({
 *   activityId: null,  // enabled: false로 동작
 *   year: 2026,
 *   month: 1,
 * });
 * ```
